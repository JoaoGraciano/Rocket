...
router.post('/venda',authMiddleware, async (req, res) => {
    let { nome, cpf, cidade, idade, clickedRows, valor_total, valorPago, troco } = req.body;
    const user = req.userId
    try{
        if( await Venda.findOne({ cpf }))
            return res.status(400).send({error: 'Venda não foi realizada'});

            const venda = await Venda.create({nome, cpf, cidade, idade, clickedRows, valor_total, user, valorPago, troco});
            console.log(venda)
